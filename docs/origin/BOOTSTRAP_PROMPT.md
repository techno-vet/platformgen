...
- Note: A Rancher-generated kubeconfig may authenticate successfully even if some `cluster-info` calls or cluster-wide reads return `Forbidden`. Prefer namespace-scoped `kubectl get pods|deployments|statefulsets -n <ns>` checks before assuming access is broken. The generated kubeconfig keeps the same Rancher-backed RBAC and does not bypass restrictions on resources like Flux `HelmRelease` objects in `tenant-flux-ns`.
