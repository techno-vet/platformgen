# Origin — How Genny Was Built

This folder contains the complete origin story of the Genny Platform: the original prompt that started it all, and the full chat history of the AI-assisted development session that built the platform from scratch.

**The entire Genny Platform was built 100% through AI-assisted development.** No code was written manually — every file, every fix, every feature came from a conversation with GitHub Copilot (`genny`).
...
## For New AI Agents (GitHub Copilot / genny)
...
Key things to know:
- You are the **Genny AI Agent** embedded inside the Genny Platform
- Your purpose is to help SREs by answering questions AND generating new widgets dynamically
- Generated widgets are `tk.Frame` subclasses saved to `genny/ui/widgets/` and hot-reloaded at runtime
- All credentials live in `~/.genny/.env` — never hardcode secrets
- The platform runs in Docker (`scripts/docker-run.sh`) or pip-installed (`genny start`)
...
1. **Bootstrap** — Full Tkinter framework: tabbed content area, hot-reload, markdown widget, Ask Genny panel, API Key Configurator widget
2. **Packaging** — Converted to proper Python package (`pyproject.toml`, `setup.cfg`, `genny` CLI entry point)
...
5. **`.env` unification** — All three run modes (pip, docker-compose, docker-run) share `~/.genny/.env`
6. **UID passthrough** — Container runs as host UID to allow bind-mounted `~/.genny` to be writable
