...
LAUNCH_SCRIPT = str(SCRIPT_DIR / "genny-launch.sh")
...
# instance to write to a stable ~/.genny/ path that is overwritten in-place instead.
...
def _open_quick_ask():
    ...
    def _run_popup():
        ...
        # Header
        tk.Label(root, text="⚡ Ask Genny", bg=BG, fg=ACCENT,
...
def _update_dialog(icon):
    ...
    root = tk.Tk()
    root.title("Genny — Check for Update")
...
def _restart_with_new_image(icon):
    ...
    icon.title = "Genny — Drill Down With Auger (Restarting with new image…)"
    try:
...
def _open_auger(icon, item):
    ...
    def _worker():
        if _container_running():
            icon.icon = DRILL_YELLOW
            icon.title = "Genny — Drill Down With Auger (Opening existing window…)"
            if not _activate_existing_platform():
                _launch_platform()
        else:
            icon.icon = DRILL_YELLOW
            icon.title = "Genny — Drill Down With Auger (Starting platform…)"
            _launch_platform()
        time.sleep(5)
        _refresh_status(icon)
...
def _toggle_keepalive(icon, item):
    icon.icon = DRILL_YELLOW
    icon.title = "Genny — Drill Down With Auger (Toggling keepalive…)"

    def _worker():
        current = _keepalive_status()
        if current.get("status") != "ok":
            icon.title = "Genny — Drill Down With Auger (Keepalive unavailable)"
            _refresh_status(icon)
            try:
                icon.update_menu()
            except Exception:
        ...
        if result.get("status") == "ok":
            ...
            icon.title = f"Genny — Drill Down With Auger (Keepalive {state})"
        else:
            icon.title = "Genny — Drill Down With Auger (Keepalive toggle failed)"
...
def _restart_auger(icon, item):
    ...
    icon.icon = DRILL_YELLOW
    icon.title = "Genny — Drill Down With Auger (Restarting…)"

    def _worker():
        if not _personalized_image_present():
            icon.title = "Genny — Rebuilding personalized image…"
            _launch_platform()
...
def _restart_daemon(icon, item):
    ...
    """
    icon.icon = DRILL_YELLOW
    icon.title = "Genny — Drill Down With Auger (Restarting Daemon…)"
...
def _full_restart_auger(icon, item):
    ...
    icon.icon = DRILL_YELLOW
    icon.title = "Genny — Drill Down With Auger (Full Restart…)"
...
def _stop_auger(icon, item):
    ...
    if _daemon_healthy():
        icon.title = "Genny — Drill Down With Auger (Tray ready — platform stopped)"
    else:
        icon.title = "Genny — Drill Down With Auger (Stopped)"
...
def _refresh_status(icon):
    if _daemon_healthy() and _container_running():
        ...
        icon.title = "Genny — Drill Down With Auger (Running)"
    elif _daemon_healthy():
        icon.icon  = DRILL_YELLOW
        icon.title = "Genny — Drill Down With Auger (Tray ready — platform stopped)"
    else:
        icon.icon  = DRILL_RED
        icon.title = "Genny — Drill Down With Auger (Daemon unavailable)"
...
def main():
    menu = pystray.Menu(
        ...
        pystray.MenuItem("💬 Quick Ask Genny…",         _quick_ask),
    ...
    icon = pystray.Icon(
        name="auger",
        icon=DRILL_YELLOW,
        title="Genny — Drill Down With Auger",
        menu=menu,
    )
