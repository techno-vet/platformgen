#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
# Genny Platform — One-Shot Launcher
# Pull the latest image from Artifactory and start Auger.
# The first-run wizard will guide you through Copilot token setup.
#
# Usage:
#   bash genny-launch.sh               # Docker mode (default)
#   bash genny-launch.sh --venv        # Native venv mode (no Docker required)
#   bash genny-launch.sh --venv --install-only   # Install deps only, don't start
# ─────────────────────────────────────────────────────────────────────────────
set -e
set -o pipefail
...
start_progress_dialog() {
    ...
    if [ -n "${DISPLAY:-}" ] && [ -f "$SCRIPT_DIR/startup_progress.py" ]; then
        nohup python3 "$SCRIPT_DIR/startup_progress.py" \
            ...
            --title "Genny Startup" >/dev/null 2>&1 &
    fi
}
...
trap 'rc=$?; if [ "$rc" -ne 0 ] && [ -f "$PROGRESS_LOG" ]; then progress_error "Genny startup failed."; fi' EXIT

echo ""
echo "╔══════════════════════════════════════════════════════╗"
echo "║          Genny Platform — Launcher               ║"
echo "╚══════════════════════════════════════════════════════╝"
echo ""
...
if [ "$VENV_MODE" -eq 1 ]; then
    ...
    if [ "$INSTALL_ONLY" -eq 1 ]; then
        ...
        echo "   To start Auger:  bash $SCRIPT_DIR/genny-launch.sh --venv"
        exit 0
    fi
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        ...
        if [ -z "$_INIT_TOKEN" ]; then
            ...
            echo "   Using token from ~/.genny/.env"
        fi
...
# ~/.genny/.env. Only fall back to an interactive prompt if no saved credential
...
if [ "$_art_authenticated" = false ]; then
    ...
    if ! art_image_access_ok "$BASE_IMAGE"; then
        ...
        echo "    Prefer ARTIFACTORY_API_KEY in ~/.genny/.env."
...
if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER}$"; then
    progress_msg "Genny container exited during startup. Recent container logs:"
    docker logs --tail 120 "$CONTAINER" 2>&1 | tee -a "$PROGRESS_LOG" || true
    progress_error "Genny container failed to start. Check docker logs auger-platform."
    exit 1
fi
...
# This runs from genny-launch.sh (the install_wizard path) so alpha testers get
...
try:
    from genny.ui.icons import install_app_icon
...
Exec=bash ${SCRIPT_DIR}/genny-launch.sh
Icon=${ICON_PATH}
Terminal=false
...
Exec=bash ${SCRIPT_DIR}/start-genny-tray.sh
Icon=${ICON_PATH}
Hidden=false
NoDisplay=false
...
progress_msg "Genny startup complete."
progress_done

# ── 14. Start System Tray Applet ─────────────────────────────────────────────
DISPLAY="${DISPLAY_VAL}" bash "$SCRIPT_DIR/start-genny-tray.sh"
