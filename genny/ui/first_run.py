"""
First-run setup wizard for Genny Platform.

Shown once when GHE_TOKEN is not configured in ~/.genny/.env.
...
def _write_ghe_token(token: str):
    """Write or update GHE_TOKEN in ~/.genny/.env."""
...
def _test_copilot(token: str) -> tuple[bool, str]:
    """Test that genny (gh copilot) works with the token. Returns (ok, message)."""
    auger_bin = _find_auger()
    if not auger_bin:
        return False, "genny CLI not found — ensure it is installed"
    env = dict(os.environ)
    env["GH_TOKEN"] = token
    env["GHE_TOKEN"] = token
    ...
    try:
        ...
        return False, f"genny returned exit code {result.returncode}: {combined[:120]}"
    except subprocess.TimeoutExpired:
        return False, "Copilot test timed out (>20s)"
    except Exception as e:
        return False, f"Error running genny: {e}"


def _find_auger() -> str | None:
    import shutil
    return shutil.which("genny") or (
        str(Path.home() / ".local/bin/genny")
        if (Path.home() / ".local/bin/genny").exists() else None
    )
...
class FirstRunWizard(tk.Toplevel):
    ...
    def __init__(self, parent: tk.Tk):
        ...
        self.parent = parent
        self.title("Genny Platform — First-Time Setup")
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="  Genny Platform — First-Time Setup",
    ...
    def _step_welcome(self):
        ...
        tk.Label(self._body, text="Welcome to Genny Platform",
                 font=("Segoe UI", 16, "bold"), fg=GREEN, bg=BG).pack(pady=(8, 4))

        tk.Label(self._body,
                 text="Genny uses GitHub Copilot as its AI backbone.\n"
                      "This wizard will configure your GitHub token so\n"
                      "Ask Genny can help you set up everything else.",
    ...
    def _step_token(self):
        ...
        tk.Label(self._body,
                 text="Paste your GitHub Personal Access Token below.\n"
                      "It will be saved to ~/.genny/.env on your host.",
    ...
    def _step_copilot(self):
        ...
        tk.Label(self._body, text="Testing Ask Genny",
                 font=("Segoe UI", 14, "bold"), fg=GREEN, bg=BG).pack(pady=(0, 8))

        self._copilot_msg = tk.StringVar(value="Launching genny CLI test...")
    ...
    def _do_copilot_test(self, token: str):
        self.after(0, lambda: self._copilot_append("$ genny ping\n"))
    ...
    def _on_copilot_done(self, ok: bool, msg: str):
        if ok:
            ...
            self._status_var.set("Ready to use Ask Genny")
            ...
            self._status_var.set("You can still proceed — Ask Genny may work once network is ready")
    ...
    def _step_done(self):
        ...
        self._btn_back.config(state=tk.DISABLED)
        self._btn_next.config(text="Open Genny  ", state=tk.NORMAL,
        ...
        tk.Label(self._body,
                 text="GitHub Copilot is configured.\n"
                      "Ask Genny can now help you configure everything else\n"
    ...
    def _on_close_attempt(self):
        ...
        if self._completed or self._current_step == "done":
            ...
            if mb.askyesno(
                "Skip Setup?",
                "Setup is not complete. You can still use Genny but Ask Genny\n"
                "won't work until GHE_TOKEN is configured in ~/.genny/.env.\n\n"
                "Skip setup?",
                parent=self
            ):
                self.grab_release()
