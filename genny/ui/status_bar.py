#!/usr/bin/env python3
"""
status_bar.py - Genny platform status bar.
...
def _read_image_tag():
    ...
    if not tag:
        for p in [Path("/.docker-image-tag"), Path.home() / ".genny" / "image_tag"]:
            try:
...
class GennyStatusBar(tk.Frame):
    ...
    def __init__(self, parent, content_ref=None, **kwargs):
        ...
        self._lbl_ver = tk.Label(
            left, text=f"Genny v{self._version}",
    ...
    def _click_image(self, _event=None):
        if not self._content:
            return
        try:
            from genny.ui.widgets.artifactory import ArtifactoryWidget
