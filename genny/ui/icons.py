"""Shared PIL-based icon factory for Genny UI widgets.
...
# ---------------------------------------------------------------------------
# Genny app icon — drill bit, used for window titlebar + dock
# ---------------------------------------------------------------------------

def make_drill_icon(size: int = 64, color: str = "#2ea043") -> Image.Image:
    """Genny A-lettermark icon with drill tip — works at 16px to 256px.
...
def install_app_icon(icon_path: str = None) -> str:
    """Save the bundled Genny app icon to icon_path (default ~/.local/share/icons/)."""
    if icon_path is None:
        dest = Path.home() / ".local" / "share" / "icons" / "genny-platform.png"
    else:
        dest = Path(icon_path)
