...
Tools are registered in ~/.genny/host_tools.json and launched via the Host Tools Daemon.
"""
...
class HostToolsWidget(tk.Frame):
    """Manage and launch host tools from within the Genny container."""
    ...
    def _create_ui(self):
        ...
        tk.Label(header,
                 text="Launch tools on your host machine directly from the Genny container.",
    ...
    def _refresh_thread(self, auto_detect_if_empty=False):
        try:
            from genny.tools.host_cmd import list_tools, auto_detect_tools
            tools = list_tools()
    ...
    def _load_host_icon(self, key: str, icon_name: str, label: tk.Label):
        ...
        try:
            from genny.tools.host_cmd import get_tool_icon
    ...
    def _launch_tool(self, key):
        ...
        def _thread():
            try:
                from genny.tools.host_cmd import launch_tool
    ...
    def _auto_detect(self):
        ...
        def _thread():
            try:
                from genny.tools.host_cmd import auto_detect_tools
                tools = auto_detect_tools()
                self.after(0, lambda: self._render_tools(tools))
    ...
    def _from_launcher(self):
        ...
        def _thread():
            try:
                from genny.tools.host_cmd import list_desktop_apps
    ...
    def _show_launcher_picker(self, apps):
        ...
        def add_selected():
            ...
            if not indices:
                ...
                return
            from genny.tools.host_cmd import register_tool
            added = []
            for i in indices:
                app = filtered_apps[i]
    ...
    def _remove_tool(self, key):
        ...
        if not messagebox.askyesno("Remove Tool", f"Remove '{key}' from host tools?"):
            return
        tools_file = _auger_home() / '.genny' / 'host_tools.json'
        try:
    ...
    def _show_tool_form(self, existing_tool):
        ...
        def find_binary():
            ...
            def _thread():
                try:
                    from genny.tools.host_cmd import find_tool
    ...
    def _quick_add(self, known_tool, parent_dialog):
        ...
        def _thread():
            try:
                from genny.tools.host_cmd import find_tool, register_tool
                binary = ''
