...
class PannerWidget(tk.Frame):
    ...
    def _find_node_dir(self):
        ...
        baked = _auger_home() / "genny-platform" / "auger_baked" / "ui" / "widgets"
        if (baked / "node_modules").exists():
            return baked
        return self.widget_dir  # fallback (will show install warning)

    def _show_error(self, title, message):
        """Show a copyable error dialog with 'Copy to Ask Genny' support."""
        _CopyableErrorDialog(self.winfo_toplevel(), title, message,
                             ask_genny_cb=self._copy_to_ask_genny)

    def _copy_to_ask_genny(self, text):
    ...
    def _stream_logs_thread(self, from_time=None, to_time=None, append=False):
        ...
        output_dir = _auger_home() / '.genny' / 'panner'
...
class _CopyableErrorDialog(tk.Toplevel):
    ...
    def __init__(self, parent, title, message, ask_genny_cb=None):
        ...
        if ask_genny_cb:
            def _copy_ask():
                ask_genny_cb(message)
                _ask_btn.config(text="✓ Copied for Ask Genny!")
                self.after(1800, lambda: _ask_btn.config(text="Copy to Ask Genny"))

            _ask_btn = tk.Button(btn_frame, text="Copy to Ask Genny",
                                 bg=ACCENT, fg='white', font=('Segoe UI', 9, 'bold'),
