...
class GitHubWidget(tk.Frame):
    ...
    def __init__(self, parent, context_builder_callback=None, **kwargs):
        ...
        # assist org filter — persisted to ~/.genny/config.yaml
    ...
    def _load_assist_org_pref(self) -> bool:
        """Read github.assist_org_only from ~/.genny/config.yaml. Defaults True."""
        try:
            import yaml as _yaml
            cfg_path = _auger_home() / '.genny' / 'config.yaml'
            if cfg_path.exists():
    ...
    def _save_assist_org_pref(self, value: bool):
        """Persist github.assist_org_only to ~/.genny/config.yaml."""
        try:
            import yaml as _yaml
            cfg_path = _auger_home() / '.genny' / 'config.yaml'
            data = {}
    ...
    def _create_issues_tab(self):
        ...
        self.issues_tree = ttk.Treeview(
            ...
            show='headings', yscrollcommand=scrollbar.set, style='Genny.Treeview'
        )
    ...
    def _open_selected_pr_file_in_browser(self):
        ...
        if not url:
            return
        from genny.tools.host_cmd import open_url as _open_url
        _open_url(url)
    ...
    def _bulk_approve_selected_open_prs(self):
        ...
        note = simpledialog.askstring(
            ...
            initialvalue="Approved via Genny Platform"
        )
        if note is None:
            return
        note = note or "Approved via Genny Platform"
    ...
    def _approve_open_pr(self):
        ...
        self._submit_open_pr_review_async(
            ...
            body=body or "Approved via Genny Platform",
            event="APPROVE",
...
"""
        ...
        from genny.tools.host_cmd import open_url as _open_url; _open_url(f"{ghe_url}/settings/tokens")
    ...
    def _open_repo_in_browser(self):
        ...
        if self.current_repo:
            from genny.tools.host_cmd import open_url as _open_url; _open_url(self.current_repo.html_url)
    ...
    def _on_issue_double_click(self, event):
        ...
        from genny.tools.host_cmd import open_url as _open_url; _open_url(issue.html_url)
    ...
    def _on_pr_double_click(self, event):
        ...
        from genny.tools.host_cmd import open_url as _open_url; _open_url(pr.html_url)
    ...
    def _on_commit_double_click(self, event):
        ...
        from genny.tools.host_cmd import open_url as _open_url; _open_url(commit.html_url)
    ...
    def _on_action_double_click(self, event):
        ...
        from genny.tools.host_cmd import open_url as _open_url; _open_url(run.html_url)
