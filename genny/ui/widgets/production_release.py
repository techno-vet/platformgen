...
def _shared_atlassian_session(instance_url: str):
    ...
    load_dotenv(Path.home() / '.genny' / '.env', override=True)
...
# Color scheme matching the Genny platform
...
class ProductionReleaseWidget(ttk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.db_path = Path.home() / '.genny' / 'logs' / 'deployments.db'
    ...
    def _load_from_confluence(self):
        ...
        # Load credentials from ~/.genny/.env
        from dotenv import load_dotenv
        load_dotenv(Path.home() / '.genny' / '.env', override=True)
        ...
        if not token and not has_jira_cookies:
            response = messagebox.askyesno(
                ...
                "No Jira MFA cookies or Confluence token were found in ~/.genny/.env.\n\n"
    ...
    def _open_pr_url(self, event=None):
        ...
        if row and row[0]:
            import webbrowser
            from genny.tools.host_cmd import open_url as _open_url; _open_url(row[0])
