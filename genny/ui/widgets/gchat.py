...
Two webhook scopes:
  PERSONAL — stored in ~/.genny/.env as GCHAT_WEBHOOK_<NAME>
             Private to each user, never committed to git. Shown with 👤
  SYSTEM   — stored in genny/data/gchat_webhooks.yaml
...
try:
    from genny.ui.utils import make_text_copyable, auger_home as _auger_home
except ImportError:
...
_ENV_FILE    = _auger_home() / '.genny' / '.env'
...
def _load_personal() -> dict:
    """Return {name: url} from ~/.genny/.env."""
    ...
    if set_key:
        try:
            ...
            raise PermissionError(
                ...
                "Fix: relaunch Genny from terminal:\n"
                "  docker rm -f genny-platform && bash ~/repos/platformgen/scripts/genny-launch.sh"
            )
...
def _find_git_root() -> Path | None:
    ...
    candidate = _auger_home() / 'repos' / 'platformgen'
        ...
        try:
            ...
            r = requests.post(f'{api}/git/commits', headers=headers,
                              json={
                                  ...
                                  'tree': new_tree,
                                  'parents': [head_sha],
                                  'author': {'name': env.get('GHE_USERNAME', 'genny'),
                                             'email': f'{env.get("GHE_USERNAME","genny")}@users.noreply.github.com'},
                              }, verify=False, timeout=20)
...
class GChatWidget(tk.Frame):
    ...
    def _build_send_tab(self):
        ...
        for lbl, fn in [('Active PR', self._fill_pr),
                         ...
                         ('Genny Invite', self._fill_invite)]:
    ...
    def _build_hooks_tab(self):
        ...
        for val, lbl, tip in [('personal', '👤 Personal', 'Saved to ~/.genny/.env — private to you'),
        ...
        self._scope_tip = tk.Label(fields, text='Saved to ~/.genny/.env — private to you',
    ...
    def _on_scope_change(self):
        ...
        if is_sys:
            ...
            self._scope_tip.config(text='Saved to ~/.genny/.env — private to you')
    ...
    def _rebuild_list(self):
        ...
        # Personal section
        if self._personal:
            self._section_header('👤 Personal  (stored in ~/.genny/.env)')
            for name in sorted(self._personal):
                self._list_row(name, self._personal[name], 'personal', '#2a3a2a')

        # System section
        if self._system:
            self._section_header('🌐 System  (shared via git · genny/data/gchat_webhooks.yaml)')
    ...
    def _form_test(self):
        ...
        def _do():
            ok, msg = _post_message(url, 'Genny webhook test: ' + name)
    ...
    def _auto_git_commit(self):
        ...
        if repo is None:
            self._git_note.config(
                text='  [!] Could not find git repo. Run: git add genny/data/gchat_webhooks.yaml && git commit',
                fg=WARN)
        ...
        if _is_feature_branch(branch):
            ...
            self._git_note.config(
                text=f'  [i] On branch "{branch}" (not a feature branch). '
                     f'Run: git add genny/data/gchat_webhooks.yaml && git commit && git push',
                fg=WARN)
            self._git_note.pack(anchor='w', padx=8, pady=(2, 4))
        else:
            self._git_note.config(
                text='  [i] Could not detect branch. Run: git add genny/data/gchat_webhooks.yaml && git commit',
                fg=WARN)
            self._git_note.pack(anchor='w', padx=8, pady=(2, 4))
    ...
    def _fill_pr(self):
        try:
            import subprocess
            repo   = str(_auger_home() / 'repos' / 'platformgen')
            ...
            text = (f'PR Review Request\nBranch: {branch}\nLast commit: {commit}\n'
                    f'https://github.helix.gsa.gov/assist/platformgen/compare/{branch}\n'
    ...
    def _fill_task(self):
        try:
            import sqlite3
            conn = sqlite3.connect(str(_auger_home() / '.genny' / 'tasks.db'))
    ...
    def _fill_invite(self):
        self._set_text('Genny Platform - Alpha\n'
                       'Get started: https://github.helix.gsa.gov/assist/platformgen\n'
                       'Run bash genny-launch.sh to install.\nQuestions? Ask in this space')
