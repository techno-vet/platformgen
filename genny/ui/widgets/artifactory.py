...
Credentials are read from ~/.genny/.env:
...
class ArtifactoryWidget(tk.Frame):
    ...
    def _connect_thread(self):
        try:
            ...
            self.after(0, lambda: messagebox.showerror(
                "Artifactory", f"Cannot connect:\n{e}\n\nCheck ARTIFACTORY_* in ~/.genny/.env"))
    ...
    def _docker_env(self):
        """Return env dict with DOCKER_CONFIG pointing to writable ~/.genny/.docker."""
        env = os.environ.copy()
        env['DOCKER_CONFIG'] = str(_auger_home() / '.genny' / '.docker')
        return env
    ...
    def _docker_login(self):
        ...
        if not registry or not cfg['username'] or not cfg['password']:
            return False, 'Missing ARTIFACTORY credentials in ~/.genny/.env'
        r = subprocess.run(
