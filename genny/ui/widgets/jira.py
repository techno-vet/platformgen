...
class JiraWidget(tk.Frame):
    ...
    def _open_url_via_daemon(self, url: str):
        ...
        def _run():
            try:
                from genny.tools.host_cmd import open_url
                open_url(url)
            except Exception as e:
    ...
    def _init_session(self):
        def _check():
            try:
                from genny.tools.jira_session import JiraSession
                j = JiraSession()
    ...
    def _show_login_progress(self):
        ...
        def _thread():
            try:
                from genny.tools.host_cmd import jira_login_stream
    ...
    def _show_detail(self, issue: dict | None, transitions: list):
        try:
            ...
            log = _auger_home() / '.genny' / 'jira_widget.log'
