...
Two-tier knowledge architecture:
  1. Static YAML   — genny/data/widget_manifests.yaml  (all widgets; Genny-editable)
  2. Learned tier  — ~/.genny/widget_knowledge/{widget_name}.yaml  (runtime discoveries)

For NEW widgets (which Genny creates and owns, not root-owned legacy widgets):
...
Usage:
    from genny.tools.widget_knowledge import save_learned
...
_LEARNED_DIR = Path.home() / ".genny" / "widget_knowledge"

_YAML_CANDIDATES = [
    Path(__file__).parent.parent / "data" / "widget_manifests.yaml",
    Path.home() / "repos" / "platformgen" / "genny" / "data" / "widget_manifests.yaml",
]
...
def get_manifest(widget_name: str) -> dict:
    ...
    """
    # Try module constant first (for new widgets Genny created)
    try:
        import importlib
        mod = importlib.import_module(f"genny.ui.widgets.{widget_name}")
