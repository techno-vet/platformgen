...
"""
Auger Platform CLI
Command-line interface for Auger AI-powered SRE platform

Dual-mode behavior:
- With subcommands: auger init, auger start, etc.
- Without subcommands: auger "question" (quick ask mode)
"""
...
def _write_session_snapshot(user_prompt: str, response_lines: list) -> None:
    ...
    try:
        hist = Path.home() / '.auger' / 'chat_history.jsonl'
        if hist.exists():
            import json as _j
            turns = []
    ...
    try:
        snap_path = Path.home() / '.auger' / '.session_snapshot.json'
...
def _build_context_preamble() -> str:
    ...
    - If ~/.auger/.session_snapshot.json exists and is < 48h old: return
...
def run_copilot_ask(prompt_text=None):
    ...
    def run_copilot(prompt):
        ...
        # Build env, loading ~/.auger/.env if it exists
        env = os.environ.copy()
        ...
        lock_path = Path.home() / '.auger' / '.copilot.lock'
        ...
        session_id_file = Path.home() / '.auger' / '.session_id'
        ...
        chat_history = Path.home() / '.auger' / 'chat_history.jsonl'
        ...
        try:
            with open(lock_path, 'r+' if lock_path.exists() else 'w') as lock_fh:
                ...
                try:
                    ...
                    click.echo(
                        '⏳ Another Ask Auger request is already processing. '
                        'Wait for it to finish before sending a new prompt. '
                        '(If this is stale, remove ~/.auger/.copilot.lock)'
                    )
                ...
                try:
                    ...
                    # the pinned ~/.auger/.session_id (keep it — it holds context).
                    ...
                    if _re.search(r'CAPIError.*400|400.*Bad Request', _full_out):
                        ...
                        if _re.search(r'CAPIError.*400|400.*Bad Request', _full_out):
                            ...
                            print(
                                ...
                                '      copilot auth login\n'
                                '    Your session pin (~/.auger/.session_id) is preserved.',
                                flush=True
                            )
                    # ── end auto-recovery ────────────────────────────────────
    ...
    if prompt_text:
        ...
        try:
            ...
            click.echo('  auger "your question here"')
            sys.exit(1)
...
def main(ctx):
    ...
    Dual-mode usage:
      ...
      auger init              # CLI mode: Initialize configuration
      auger start             # CLI mode: Start GUI
      auger "question"        # Ask mode: Quick Copilot query
      auger                   # Ask mode: Open GUI prompt
...
@click.option('--token', prompt='GitHub Copilot token (github.com)', 
              help='GitHub Copilot token for Ask Auger')
@click.option('--config-dir', default=None, 
              help='Custom config directory (default: ~/.auger)')
...
def init(token, config_dir, datadog_api_key, datadog_app_key):
    ...
    IMPORTANT: Use your Copilot token (github.com) for Ask Auger.
    Enterprise GitHub and other integrations can be configured later
    via Ask Auger or by editing ~/.auger/.env.
    
    Priority: Get Ask Auger working first, then configure everything else!
    """
    from auger.config_manager import AugerConfigManager
    
    if not config_dir:
        config_dir = Path.home() / '.auger'
    else:
        config_dir = Path(config_dir)
    
    click.echo(f"📁 Initializing Auger in: {config_dir}")
    ...
    click.echo("\n✅ Auger initialized successfully!")
    click.echo(f"📁 Config directory: {config_dir}")
    click.echo(f"📄 Config file: {config_dir / 'config.yaml'}")
    click.echo(f"🔐 Secrets file: {config_dir / '.env'}")
    
    click.echo("\n🚀 Next steps:")
    click.echo("  1. Start Auger: auger start")
    click.echo("  2. Open Ask Auger chat panel")
...
def start(port, display, debug, config_dir):
    ...
    Launches the Auger Platform with all enabled widgets.
    """
    ...
    if not config_file.exists():
        click.echo("❌ Auger not initialized!")
        click.echo("Run: auger init")
        sys.exit(1)
    
    click.echo("🚀 Starting Auger Platform...")
...
def config(config_dir):
    ...
    Displays the current Auger configuration (with secrets redacted).
    """
    from auger.config_manager import AugerConfigManager
...
def widgets(config_dir):
    ...
    click.echo("\n💡 Enable/disable widgets in: ~/.auger/config.yaml")


@main.command()
@click.option('--config-dir', default=None, help='Custom config directory')
def doctor(config_dir):
    """Run diagnostics on Auger installation
    ...
    click.echo("🔍 Running Auger diagnostics...")
    click.echo("=" * 70)
    ...
    if issues:
        ...
        click.echo("✅ All checks passed! Auger is ready to use.")
        click.echo("\nRun: auger start")
